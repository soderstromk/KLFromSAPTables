...
def WriteXL(Path, KLResults, TemplateID, TargetList, axdiv, minsyl, syltype, sylinterval, smoothingfactor):
    ...
    NoticeText = "iiiiiiiiiiii"  # "rowcount = " + rowcount
    window.StatusLabel.config(text=NoticeText)
    window.StatusLabel.update()
    ...
    NoticeText = "Write to Excel Step 9..."
    window.StatusLabel.config(text=NoticeText)
    window.StatusLabel.update()
...
def MakeTemplateOrTargetTable(TableName, TempOrTarg, OnlyMotifs):
    ...
    if TableName:
        ...
        query = "CREATE TABLE "+ TempOrTarg +" SELECT * FROM " + TableName + " ORDER BY recnum"
        cursor.execute(query)
        ...
        if OnlyMotifs >= 1:
            query = "ALTER TABLE " + TempOrTarg + " ADD(preint FLOAT, postint FLOAT, syll_type BIT)"
            cursor.execute(query)

            query = "UPDATE " + TempOrTarg + " AS t1 INNER JOIN " + TableName[5:] + "_SYLDUR AS t2 ON t1.recnum = t2.recnum SET t1.preint = t2.preint, t1.postint = t2.postint, t1.syll_type = t2.syll_type"
            cursor.execute(query)
            conn.commit()
